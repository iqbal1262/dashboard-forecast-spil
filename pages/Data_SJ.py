...
# --- KONFIGURASI HALAMAN ---
st.set_page_config(
    page_title="Dasbor Analisis SJ",
    page_icon="🚢",
    layout="wide"
)

# --- FUNGSI HALAMAN 1: ANALISIS SJ KAPAL ---
def display_SJ_kapal():
    ...
    def load_data_from_gsheets(spreadsheet_id, tab_id):
        ...
        try:
            df = pd.read_csv(
                url, index_col='SJ_CREATED_ON', parse_dates=True,
                na_values=['', ' ', '-', 'NA', 'N/A']
            )
    ...
    # --- DATA SOURCE ---
    SPREADSHEET_ID = "1FRnseeg5FHrciZ_CyU20QpI1YrePhATpUFvSuKbRVrA"
    ...
    # --- MEMUAT DATA ---
    with st.spinner("Memuat data untuk Analisis SJ Kapal..."):
    ...
    # --- KONTEN HALAMAN ---
    st.header("Dasbor Hasil Analisis - SJ Kapal")
    ...
    with tab1:
        ...
        if df_aktual is not None and 'observed' in df_aktual.columns:
            ...
            with col2:
                ...
                st.markdown("""
                - **Biaya mingguan sangat volatil** dengan beberapa lonjakan ekstrem yang terjadi secara periodik, terutama di pertengahan tahun.
                - Range nilai pengeluaran sangat lebar, yaitu mulai **Rp107 Juta hingga Rp8,7 Miliar**.
                """)
        else:
    ...
    with tab2:
        ...
        with table_col:
            ...
            if df_forecast is not None:
                ...
                df_forecast_display = df_forecast_display.rename(columns={'SJ_CREATED_ON': 'SJ_CREATED_ON', 'prediksi_inti': 'Forecast', 'batas_bawah_99': 'Batas Bawah', 'batas_atas_99': 'Batas Atas'})
                ...
                st.dataframe(df_forecast_display, column_config={"SJ_CREATED_ON": st.column_config.DateColumn("SJ_CREATED_ON", format="DD MMM YYYY")}, hide_index=True, use_container_width=True)
    ...
    with tab3:
        ...
        def create_unified_display_df(df_aktual, df_train, df_test, df_forecast):
            ...
            unified_df = unified_df.reset_index().rename(columns={'index': 'SJ_CREATED_ON'})
            unified_df = unified_df[['SJ_CREATED_ON', 'Keterangan', 'Aktual', 'Prediksi']]
            return unified_df
        ...
        st.dataframe(unified_df, column_config={"SJ_CREATED_ON": st.column_config.DateColumn("SJ_CREATED_ON", format="DD MMM YYYY")}, use_container_width=True, hide_index=True)

# --- FUNGSI UNTUK HALAMAN DUMMY SJ Deck ---
# --- FUNGSI HALAMAN 2: ANALISIS SJ DECK ---
def display_SJ_deck():
    # --- FUNGSI LOADING DATA (SAMA PERSIS) ---
    @st.cache_data
    def load_data_from_gsheets(spreadsheet_id, tab_id):
        url = f'https://docs.google.com/spreadsheets/d/{spreadsheet_id}/export?format=csv&gid={tab_id}'
        try:
    ...
    # --- DATA SOURCE SJ DECK ---
    SPREADSHEET_ID = "1AI5b45gnNlKD9jl--KRKc6l8RwEpjZuMm_1BIUKBSuU"
    TAB_ID_AKTUAL = "542042387"
    TAB_ID_TRAIN = "1217230565"
    TAB_ID_TEST = "1677129148"
    TAB_ID_FORECAST = "0"
    ...
    # --- MEMUAT DATA ---
    with st.spinner("Memuat data untuk Analisis SJ Deck..."):
        df_aktual = load_data_from_gsheets(SPREADSHEET_ID, TAB_ID_AKTUAL)
        df_train = load_data_from_gsheets(SPREADSHEET_ID, TAB_ID_TRAIN)
        df_test = load_data_from_gsheets(SPREADSHEET_ID, TAB_ID_TEST)
        df_forecast = load_data_from_gsheets(SPREADSHEET_ID, TAB_ID_FORECAST)
    ...
    # --- KONTEN HALAMAN ---
    st.header("Dasbor Hasil Analisis - SJ Deck")

    tab1, tab2, tab3 = st.tabs(["🧩 Dekomposisi Data", "📊 Forecast Result", "📄 Tinjau Data"])

    # >>>>>>>> TAB 1 (copy dari SJ Kapal) <<<<<<<<
    with tab1:
        st.subheader("Total Pengeluaran Mingguan (Data Aktual)")
        if df_aktual is not None and 'observed' in df_aktual.columns:
            col1, col2 = st.columns([3, 1])
            ...
            with col2:
                st.write("#### 📝 Ringkasan Analisis")
                st.markdown("""
                - Pola pengeluaran mingguan **fluktuatif** dengan kecenderungan sebagian besar transaksi berada di kisaran **Rp500 juta – Rp1,5 miliar**.
                - **Terdapat beberapa lonjakan tajam**, misalnya awal 2023, awal 2024, dan pertengahan 2025, dengan nilai mencapai sekitar Rp 2–3 miliar. Lonjakan ini tampak sebagai **outlier musiman atau transaksi skala besar**.
                """)
        else:
            st.warning("Kolom 'observed' untuk data aktual tidak ditemukan.")
        st.markdown("---")
...
st.sidebar.title("Dasbor Analisis SJ")

# Navigasi dropdown dipindahkan ke sidebar
pilihan_SJ = st.sidebar.selectbox(
    "Pilih Jenis Data:",
    ("SJ Kapal", "SJ Deck")
)
...
if pilihan_SJ == "SJ Kapal":
    display_SJ_kapal()  # Panggil fungsi untuk menampilkan analisis kapal

elif pilihan_SJ == "SJ Deck":
    display_SJ_deck()  # Panggil fungsi untuk halaman dummy